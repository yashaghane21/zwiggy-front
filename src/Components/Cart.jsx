...
const Cart = () => {
  ...
  return (
    <div className='flex flex-row   absolute sm:left-[10px] h-screen mt-[50px]'>
      <div className='items-center justify-center bg-slate-30'>
       {
        (cart.length == 0) ?
      <>
           <img src={ecart} alt='ddf' className='md:h-[500px] md:w-full w-[300px] h-[250px] pl-[80px]' />
        </>:  cart?.map((item, index) => (
          <div className=' border-2  border-gray-200 rounded-2xl gap-4 m-2 pb-4 hover:shadow-2xl '>
            <div key={index} className='h-70   mx-auto md:flex rounded-2xl '>
              <img src={item.img} alt='fe' className='h-48 w-full md:w-48 sm:h-full p-1 rounded-2xl object-cover sm:w-48 ' />
              <div className='pl-5'>
        ...
        <div className=" absolute md:left-[700px] sm:left-[1000] md:top-[200px] " >
